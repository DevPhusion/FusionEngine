...
#include "../../../../Header Files/Core/Files/Export/ExportPackageCrypto.h"
#include <fstream>
...
bool ExportPackageReader::Load(const std::filesystem::path& packPath) {
	...
	for (auto& e : raw) {
		...
		ExportPackageCrypto::XorBuffer(chunk, key);
		entries[e.path] = std::move(chunk);
	}
	return true;
}

const std::vector<uint8_t>* ExportPackageReader::Get(const std::string& virtualPath) const {
	auto it = entries.find(virtualPath);
	return it != entries.end() ? &it->second : nullptr;
}


bool ExportPackageReader::HasModule(const std::string& moduleDotted) const {
	return entries.count(ModuleNameToVirtualPath(moduleDotted)) != 0;
}

bool ExportPackageReader::IsPackage(const std::string& moduleDotted) const {
	std::string path = moduleDotted;
	std::replace(path.begin(), path.end(), '.', '/');
...
std::string ExportPackageReader::GetSource(const std::string& moduleDotted) const {
